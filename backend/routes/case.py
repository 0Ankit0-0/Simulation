...
case_schema = {
    ...
    'case_type': {'type': 'string', 'allowed': ['civil', 'criminal', 'family', 'corporate'], 'required': True}
}
...
def submit_case():
    ...
    try:
        ...
        for file in files:
            try:
                ...
                if not is_valid_file:
                    failed_files.append({'filename': file.filename, 'error': file_error})
                    continue
                ...
                try:
                    ...
                    logger.info(f"Successfully processed file: {file.filename} for case: {case_id}")
                
                except Exception as parse_error:
                    logger.error(f"Failed to parse evidence file {file.filename}: {parse_error}")
                    # Clean up file if parsing failed
                    if os.path.exists(filepath):
                        os.remove(filepath)
                    failed_files.append({'filename': file.filename, 'error': 'Failed to process file'})
        ...
        if failed_files:
            ...
            response_data["warning"] = f"{len(failed_files)} files could not be processed"
